...
public class SendingConfirmationServiceImpl implements SendingConfirmationService {
  ...
  public boolean sendForConfirmationCustomValues(SenderService senderService, String analyticsPath,
    ...
    try {
      ...
      ModelCheckResult result = modelCheckerResultWrapper
          .getModelCheckResultList().stream()
          .filter(r -> r.getProbability() == probability && r.getExpectedMessages() == messages)
          .findAny()
          .orElseGet(() -> modelCheckerResultWrapper
              .getModelCheckResultList().stream()
              .min(Comparator.comparingDouble(r -> Math.abs(r.getProbability() - probability) + Math.abs(r.getExpectedMessages() - messages)))
              .orElseThrow(() -> new RuntimeException("There is no models")));
